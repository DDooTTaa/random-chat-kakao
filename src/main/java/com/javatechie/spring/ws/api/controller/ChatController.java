...
public class ChatController {
	...
	@SendTo("/topic/public") //구독 경로, SendTo는 1:n으로 메시지 뿌릴 때 사용하는 구조이며 보통 경로가 "/topic", SendToUser는 1:1 경로는 "/queue"
	//@Payload: websocket에서 전송하는 데이터를 받기 위한 어노테이션
	//SendTo로 토픽 구독한 사람 전원에게 메시지 보낼 수 있지만 메소드 converAndSend 메소드를 사용하면 @payload로 넘어온 메시지 내부의 값을 토픽으로 활용
